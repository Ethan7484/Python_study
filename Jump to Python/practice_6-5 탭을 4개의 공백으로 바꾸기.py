...
# 이번에는 문서 파일을 읽어서 그 문서 파일 안에 있는 탭(tab)을 공백(space) 4개로 바꾸어 주는 스크립트를 작성해보자.
...
# 예를 들어 a.txt 파일에 있는 탭을 4개의 공백으로 바꾸고 b.txt 파일에 저장하고 싶다면 다음과 같이 수행해야 한다.
"""
...
b.txt
"""
# 입력으로 전달한 a.txt b.txt가 정상적으로 출력되는 것을 확인할 수 있다.


# 3. 테스트를 위한 원본 파일(탭을 포함한 파일)인 a.txt를 다음과 같이 작성한다. 각 단어는 탭(\t) 문자로 분리되도록 입력해야 한다. 
"""
Life    is  too short
You need    python
"""
...
You    need    python
"""
# 아마도 탭 문자가 공백 4개로 변경되어 출력될 것이다. 하지만 탭과 공백의 차이점을 눈으로 알 수 없으므로 탭이 정상적으로 공백으로 되어 있는지 확인하기 어렵다.
...
f.close()
"""
# 탭이 공백으로 변경된 space_cotent를 출력 파일인 dst에 쓰도록 코드를 수정하였다.
...
python tabto4.py a.txt b.txt
"""
# 위 명령을 수행하면 b.txt 파일이 생성된다. 에디터로 b.txt 파일을 열어서 탭이 4개의 공백 문자로 변경되었는지 확인해보자. 프로그램을 작성할 때 에디터는 대부분 탭과 공백 문자를 다르게 표시하므로 눈으로 확인이 가능할 것이다.
