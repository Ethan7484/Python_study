...
"""
# two_times 함수는 리스트 요소를 입력받아 각 요소에 2를 곱한 결괏값을 돌려준다. 실행 결과는 다음과 같다. [2, 4, 6, 8]
...
"""
# 이제 앞 예제를 해석배보자. 먼저 리스트의 첫번째 요소인 1이 two_times 함수의 입력값으로 들어가고 1 * 2의 과정을 거쳐서 2가 된다. 
# 다음으로 리스트의 두 번째 요소인 2 * 2의 과정을 거쳐 4가 된다. 
# 따라서 결괏값 리스트는 이제 [2, 4]가 된다. 총 4개의 요솟값이 모두 수행되면 마지막으로 [2, 4, 6, 8]을 돌려준다. 이것이 map 함수가 하는 일이다.
...
"""
print(max([1, 2, 3]))    # 최댓값 3을 돌려줌
print("python")    # 문자열 중에 가장 끝 y를 돌려줌
"""
...
"""
print(min([1, 2, 3]))    # 최솟값 1을 돌려줌
print("python")    # 최솟값 h를 돌려줌
"""
...
### open
# open(filename, [mode])은 "파일 이름"과 "읽기 방법"을 입력 받아 파일 객체를 돌려주는 함수이다. 
# 읽기 방법(mode)를 생략하면 기본값인 읽기 전용 모드(r)로 파일 객체를 만들어 불러준다.
# w - 쓰기 모드로 파일 열기
# r - 읽기 모드로 파일 열기
# a - 추가 모드로 파일 열기
# b - 바이너리 모드로 파일 열기
...
#fread = open("append_mode.txt", 'a')



### ord
# ord(c)는 문자의 아스키 코드 값을 돌려주는 함수이다.
# >> ord 함수는 chr 함수와 반대이다. chr는 아스키 코드값을 받아 문자로 돌려준다.
...
"""
print(list(range(1, 10, 2)))    # 1부터 10까지 숫자 중에서 2개씩 건너뛰고 출력, [1, 3, 5, 7, 9]
print(list(range(0, -10, -1)))    # 0부터 -10까지의 숫자의 -1씩 출력
"""



### round  *python은 반올림할 때 짝수에 묶는 ROUND_HALF_EVEN 방식을 사용하기 때문에, 0.5를 반올림하면 짝수에 가까운 0으로 출력된다.*
# round(numberp[,ndigits]) 함수는 숫자를 입력받아 반올림 해주는 함수이다.
# >> [,digits]는 ndigits가 있을 수도 있고 없을 수도 있다는 의미이다.
"""
print(round(4.6))    # 5 출력
print(round(4.2))    # 4 출력
"""

# 다음과 같이 실수 5.678을 소수점 2자리까지만 반올림하여 표시할 수 있다.
"""
print(round(5.678, 2))    # 5.67
"""
...
"""
a = sorted([3, 1, 2])
print(a)    # 변수 a에 저장된 [3, 1, 2]를 정렬하여 [1, 2, 3]으로 출력
...
"""
a = tuple("abc")
print(a)    # ('a', 'b', 'c')를 돌려준다

a = tuple([1, 2, 3])
print(a)    # (1, 2, 3)

a = tuple((1, 2, 3))
print(a)    # 입력받은 값이 튜플이기 때문에 그래도 돌려준다. (1, 2, 3)
"""
...
# >> 여기서 사용한 *iterable은 반복 가능한(iterable) 자료형을 여러 개를 입력할 수 있다는 의미이다.
