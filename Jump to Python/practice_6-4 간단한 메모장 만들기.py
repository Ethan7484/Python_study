...
Life is too short
"""
# 입력으로 전달된 옵션과 메모 내용이 그대로 출력 되는 것을 확인할 수 있다.
...
# 여기에서 메모는 항상 새로운 내용이 작성 되는 것이 아니라 한줄씩 추가되어야 하므로 파일 열기 모드를 a로 했다. 
...
"""
# 그리고 파일에 정상적으로 메모가 기입 되었는지 다음과 같이 확인 해보자.
"""
type memo.txt
(memo.txt 파일 내용이 출력됨)
"""
...
# 5. 이번에는 작성한 메모를 출력하는 부분을 차례이다. 메모 출력은 다음과 같이 동작하도록 만들어 보자.
# >> python memo.py -v
# 메모 추가는 -a 옵션을 사용하고 메모 출력은 -v옵션을 사용한다.
...
if option == '-a':
    ...
    print(memo)
"""
# 옵션으로 -v가 들어온 경우 memo.txt 파일을 읽어서 출력한다. argv[2]에 다른 값을 입력해도 무조건 memo.txt를 읽어서 출력한다.
