...
# 첫 번째 방법은 모니터 화면 대신 파일에 결괏값을 적는 방법이다. 두 방법의 차이점은 print 대신 파일 객체 f의 write 함수를 사용한 것 말고는 없으니 바로 눈에 들어 올 것이다.

# 이제 명령 프롬프트 창에서 첫번째 예제를 실행해 보자.



### 프로그램의 외부에 저장된 파일을 읽는 여러 가지 방법
# 파이썬에서는 외부 파일을 읽어 들여 프로그램에서 사용할 수 있는 여러가지 방법이 있다.
# 이번에는 그 방법을 자세히 알아보자.

"""
readline() 함수 이용하기
"""
# 첫번째 방법은 readline 함수를 사용하는 방법이다. 다음 예를 보자.
...
f.close()
"""
# 위 예는 f.open("새파일.txt", 'r')로 파일을 읽기 모드로 연 후 "readline()"을 사용해서 파일의 첫 번째 ㅈ줄을 읽어 출력하는 경우이다.
...
f.close()
"""
# 즉 "while True:" 무한 루프 안에서 "f.readline()"을 사용해서 파일을 계속해서 한 줄씩 읽어 들인다.
...
while 1:
    ...
    print(data)
"""
# 위 예는 사용자의 입력을 바아서 그 내용을 출력하는 경우이다. 파일을 읽어서 출력하는 예제와 비교해 보자.
