...
# 사실 실제 업무에서 사용하는 페이징 기술은 훨씬 복잡한데 여기에서는 그중 간단한 총 페이지 수를 구하는 방법에 대해서만 알아보겠다.

# 1. 다음과 같이 총 건수(m)를 한 페이지에서 보여 줄 게시물 수(n)로 나누고 1을 더하면 총 페이지 수를 얻을 수 있다.
# >> 총 페이지 수 = (총 건수 / 한 페이지당 보여 줄 건수) + 1


# 2. 이러한 공식을 적용했을 경우 총 페이지 수가 표의 값처럼 구해지는지 확인해보자(m을 n으로 나눌 때 소수점 아래 자리를 버리기 위해 / 대신 //연산자를 사용하였다.)

"""
def getTotalPage(m, n):
    return m // n + 1

print(getTotalPage(5, 10))    # 총 페이지 수: 1
print(getTotalPage(15, 10))    # 총 페이지 수: 2   
print(getTotalPage(25, 10))    # 총 페이지 수: 3
print(getTotalPage(30, 10))    # 총 페이지 수: 4
"""

# 첫 번째, 두 번째, 세 번째 케이스는 공식에 맞게 결과가 출력된다. 하지만 네 번째 케이스는 총 건수가 30이고 한 페이지에 보여 줄 건수가 10인데 4가 출력되어 실패해 버렸다.
...
print(getTotalPage(15, 10))    # 총 페이지 수: 2   
print(getTotalPage(25, 10))    # 총 페이지 수: 3
print(getTotalPage(30, 10))    # 총 페이지 수: 3
